...
class SmtpMailer
{
    ...
    {
        if ($this->SMTP_Debugoutput == "error_log")
        {
            error_log($str);
        }
        else
    ...
    private function get_lines()
    {
        ...
        stream_set_timeout($this->smtp_conn, $this->SMTP_Timeout);
        if ($this->Timelimit > 0)
        {
        ...
        while (is_resource($this->smtp_conn) && !feof($this->smtp_conn))
        {
            ...
            if ($info['timed_out'])
            {
                if ($this->do_debug >= 4)
                {
                    $this->edebug("SMTP -> get_lines(): timed-out (" . $this->SMTP_Timeout . " seconds) <br />");
                }
                break;
            }
    ...
    protected function SmtpSend($header, $body)
    {
        ...
        {
            $this->SetError($this->Lang('from_failed') . $smtp_from . " : " . implode(",", $this->getError()));
    ...
    public function SmtpConnect()
    {
        ...
        try
        {
            while ($index < count($hosts) && !$connection)
            {
                ...
                if ($this->Connect(($ssl ? 'ssl://' : '') . $host, $port, $this->SMTP_Timeout))
                {
                    ...
                    if ($this->SMTPAuth)
                    {
                        if (!$this->Authenticate($this->Username, $this->Password, $this->AuthType, $this->Realm, $this->Workstation))
                        {
    ...
    public function CreateHeader()
    {
        ...
        if ($this->XMailer == '')
        {
            $result .= $this->HeaderLine('X-Mailer', 'PHPMailer ' . $this->MAILER_Version . ' (http://code.google.com/a/apache-extras.org/p/phpmailer/)');
        }
        else
        {
